...
import { CONTENT_TYPES } from "../../domain/ports/file-storage.port";
  ...
  implements TempStoragePromotionPort
{
  constructor(
    ...
    private readonly optimize_image_use_case: OptimizeImageUseCase,
  ) {
    super();
  }
  ...
  async finalize_compound_path(compound_path: string): Promise<string> {
    ...
    const [optimized] = await this.optimize_image_use_case.execute([multer_file], {
